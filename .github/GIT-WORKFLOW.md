Git workflow for yii2-sortable-input contributors
=========================================

So you want to contribute to yii2-sortable-input? Great! But to increase the chances of your changes being accepted quickly, please
...
### 1. [Fork](http://help.github.com/fork-a-repo/) the yii2-sortable-input repository on GitHub and clone your fork to your development environment 

```
git clone git@github.com:YOUR-GITHUB-USERNAME/yii2-sortable-input.git
```
...
### 2. Add the main yii2-sortable-input repository as an additional git remote called "upstream"

Change to the directory where you cloned yii2-sortable-input, normally, "yii2-sortable-input". Then enter the following command:

```
git remote add upstream git://github.com/kartik-v/yii2-sortable-input.git
```

### 3. Prepare the testing environment

- You should have a working yii 2 development environment in which you have already installed `yii2-sortable-input` and includes latest and updated `yii2-sortable-input` fork from source.
- Ensure you have the latest `dev-master` releases of all dependent extensions via your composer updates
- Ensure you use the above cloned latest `yii2-sortable-input` code in your testing environment

**Now you have a working playground for hacking on yii2-sortable-input.**
...
### 2. Fetch the latest code from the main yii2-sortable-input branch
...
### 3. Create a new branch for your feature based on the current yii2-sortable-input master branch
...
the sortable-input of the issue that your code relates to. If you aren't fixing any particular issue, just skip sortable-input.
For example:
...
`#999` is the issue sortable-input that the `Bug` or `Enh` is referring to.
The changelog should be grouped by type (`Bug`,`Enh`) and ordered by issue sortable-input.
...
Commit your changes with a descriptive commit message. Make sure to mention the ticket sortable-input with `#XXX` so GitHub will
...
### 7. Pull the latest yii2-sortable-input code from upstream into your branch
...
you should fix them now and commit the changes again. This ensures that it's easy for the yii2-sortable-input team to merge your changes
with one click.
...
and become part of the next yii2-sortable-input release. If not, don't be disheartened, different people need different features and yii2-sortable-input
